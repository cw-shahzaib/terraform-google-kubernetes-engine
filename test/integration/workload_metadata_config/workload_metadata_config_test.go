...
func TestWorkloadMetadataConfig(t *testing.T) {
	...
	bpt.DefineVerify(func(assert *assert.Assertions) {
		...
		registryProjectIds := bpt.GetStringOutput("registry_project_ids")
		serviceAccount := bpt.GetStringOutput("service_account")

		for _, registryProjectId := range registryProjectIds {
			imaPolicy := gcloud.Runf(t, "projects get-iam-policy %s", registryProjectId)
